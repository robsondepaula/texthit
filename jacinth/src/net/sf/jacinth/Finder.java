...
public class Finder extends Thread {
    ...
    public void findAll() {
        ...
        while (matcher.find(start)) {
            ...
            if (_replace != null) {
                if (!replaceAll) {
                    ReplaceOptionsDialog dlg = ReplaceOptionsDialog.getNewReplaceDialog(editor, Local.getString("Replace this occurence?"));
        ...
        JOptionPane.showMessageDialog(editor, Local.getString("Search complete")+".");
        if (showCdlg) {
            editor.toolsPanel.remove(cdlg);
            if (editor.toolsPanel.getTabCount() == 0)
                editor.hideToolsPanel();
